...
resource "nhncloud_networking_port_v2" "bastion-nic" {
  name = "web_port_1"
...
# resource "nhncloud_networking_floatingip_associate_v2" "bastion-fip_associate" {
#     floating_ip = nhncloud_networking_floatingip_v2.bastion-fip.address
#     port_id = nhncloud_networking_port_v2.bastion-nic.id
# }

# web 서버 인스턴스 생성
resource "nhncloud_compute_instance_v2" "bastion-instance" {
    name      = "bastion-server"
    key_pair  = "kips_key"
    flavor_id = data.nhncloud_compute_flavor_v2.t2c1m1.id

    network {        
        port = nhncloud_networking_port_v2.bastion-nic.id        
    }    
...
# Network Interface 생성
resource "nhncloud_networking_port_v2" "dev-nic" {
  name = "was_port_1"
  network_id = nhncloud_networking_vpc_v2.kips-vpc.id
  admin_state_up = "true"
  fixed_ip {
    subnet_id = nhncloud_networking_vpcsubnet_v2.dev-subnet.id
  }
  security_group_ids = [nhncloud_networking_secgroup_v2.dev-sg.id]
}

# dev 서버 인스턴스 생성
resource "nhncloud_compute_instance_v2" "dev-instance" {
    name      = "dev-server"
    key_pair  = "kips_key"
    flavor_id = data.nhncloud_compute_flavor_v2.t2c1m1.id

    network {        
        port = nhncloud_networking_port_v2.dev-nic.id        
    }    
